...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
	...
	for (int i = 0; i < num_particles; i++)
	{
		...
		if (yaw_rate == 0)
		{
			new_x = particles[i].x + cos(particles[i].theta)*velocity*delta_t;
			new_y = particles[i].y + sin(particles[i].theta)*velocity*delta_t;
		...
		else
		{
			...
			new_y = particles[i].y + velocity/yaw_rate*(cos(particles[i].theta) - 
					cos(yaw_rate*delta_t + particles[i].theta));
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
	...
	//   transfer observations to map coordinates for each particle
	for (int p = 0; p < num_particles; p++)
	{
		...
		{
			double d = sensor_range;
			for (int j = 0; j < map_landmarks.landmark_list.size(); j++)
			{
				double d_temp = dist(map_landmarks.landmark_list[j].x_f, map_landmarks.landmark_list[j].y_f, 
						trans_observations[i].x, trans_observations[i].y);
				if (d_temp < d)
				{
					d = d_temp;
					trans_observations[i].id = map_landmarks.landmark_list[j].id_i;
				}
			}
		}
		...
		{	
			...
			if(association!=0)
			{
				...
				long double mult = 1/(2*M_PI*std_landmark[0]*std_landmark[1])*
					exp(-1*(pow(meas_x - mu_x,2)/(2*std_landmark[0]*std_landmark[0])+
							pow(meas_y - mu_y,2)/(2*std_landmark[1]*std_landmark[1])));
				if(mult>0)
				{
					particles[p].weight *= mult;
				}
			}
			else
			{
				long double mult = 1/(2*M_PI*std_landmark[0]*std_landmark[1])*
					exp(-1*(pow(sensor_range,2)/(2*std_landmark[0]*std_landmark[0])+
							pow(sensor_range,2)/(2*std_landmark[1]*std_landmark[1])));
				if(mult>0)
				{
					particles[p].weight *= mult;
				}
